...
<div nwp-file="<%out.print( request.getAttribute("filename") );%>" class="hyella-source-container">
    <%
        ...
        if( jget.has("id") && jget.has("nwp2_action") && jget.has("workflow") ){
            $data = GetControlPanel( jget.getString("id"), table_name, jget.getString("workflow") );
            //$data = GetControlPanel( jget.getString("id"), jget.getString("nwp2_action"), jget.getString("workflow") );
        }

        //$data = GetControlPanel( "ecm1650356439210", "ecm2" );
            ...
            <h4 style="text-align:center;">
                <%
                    if( $data.getInt("show_refresh") > 0 && jget.has( "action" ) && ! jget.getString( "action" ).isEmpty() ){
                        ...
                        $rsa = "?action="+ jget.getString( "action" ) +"&workflow="+ jget.getString("workflow") +"&todo="+ jget.getString("todo")+"&nwp2_todo="+ jget.getString("nwp2_todo")+"&nwp2_action="+ jget.getString("nwp2_action");
                ...
                %>
                <a href="#" title="Re-open <% out.print( $title_text ); %>" class="custom-single-selected-record-button m-link" id="wcpanel-<% out.print( $pid ); %>" override-selected-record="<% out.print( $pid ); %>" action="<% out.print( $rsa ); %>&html_replacement_selector=<% out.print( $container ); %>"><i class="icon-refresh"></i></a>
                <% } %>
                <strong>
                    <%
                        if( ! $title_text.isEmpty() ){
                            out.print( $title_text );
                            $gparams2 += "&ctitle=" + GlobalFunctions.rawurlencode( $title_text );
                        }else {
                            if ( ! $title_key.isEmpty() ) {
                                out.print($title_key);
                                $gparams2 += "&ctitle=" + GlobalFunctions.rawurlencode( $title_key );
                            } else {
                                String dlabel = "";
                                ...
                                out.print(dlabel);
                                $gparams2 += "&ctitle=" + GlobalFunctions.rawurlencode( dlabel );
                            }
                        }     // out.print( isset( $e[ $title_key ] ) ? $e[ $title_key ] : ( isset( $e[ 'name' ] ) ? $e[ 'name' ] : $label ) . ( isset( $e["serial_num"] ) ? '#'.$e["serial_num"] : '' );
                    %></strong></h4>
        </div>
    </div>
    <% } %>
            ...
            <div class="tabbable tabbable-custom" id="transaction-tabs">
                        ...
                        try{
                            ...
                            for(int i1 = 0; i1 < $tabs.names().length(); i1++){
                                ...
                                if( $tbs.has("html") && ! $tbs.getString("html").isEmpty() ){
                                    ...
                                    $th2 += "<li ><a data-toggle=\"tab\" href=\"#tab-2-" +  $unique_key + "\" clickme=\"" +  $ktbs + "\" class=\"custom-single-selected-record-button " +  $otr +"\" action=\"?action=display_sub_menu&nwp2_action=" +  $tb_action +"&todo=&nwp2_todo=" +  $tb_todo +"&html_replacement_selector=general-activity-window-" +  $unique_key + $tab_params+"\" override-selected-record=\"" +  $xid + "\" class=\"m-link\">" +  $tb_title + "</a></li>";
                                }

                            }
                        }catch (Exception e) {
                            out.print( "No TABS FOUND" );
                        }
                    %>
                ...
                <div class="tab-content resizable-heightx" style="overflow-y:hidden; overflow-x:hidden;">
                    ...
                    <div class="tab-pane <% out.print( $d_active ); %>" id="tab-1-<% out.print( $unique_key ); %>">
                        <br />
                        <div class="row">
                            <div class="col-md-<% out.print( $details_column ); %>">
                                <%
                                    out.print( $notice );
                                    out.print( $details );
                                %>
                                ...
                                <%
                                            ...
                                            if( $agd.has("title") && $agd_actions.length() > 0 ){
                                                ...
                                                if( $agd_actions.length() > 0 ){
                                                    ...
                                                    for( int i2 = 0; i2 < $agd_actions.length(); i2++ ){
                                                        ...
                                                        $h += "<li role=\"presentation\"><a clickme="+ $kav +" " +$ti+ " role=\"menuitem\" tabindex=\"-1\" href=\"#\" action=\"?action=display_sub_menu&nwp2_action="+ $av.getString("action") +"&todo=&nwp2_todo="+ $av.getString("todo")+ "&html_replacement_selector=general-activity-window-"+ $unique_key +"&container="+ $container + $params2 + $gparams2 +"\" override-selected-record=\""+ $xid +"\" class=\"custom-single-selected-record-button open-activity-window\" unique_key=\""+ $unique_key +"\">"+ $av.getString("title") +"</a></li>";
                                                    }
                                    ...
                                    if( $general_actions.length() > 0  ){
                                        ...
                                        for( int i = 0; i < $general_actions.length(); i++ ){
                                            ...
                                            $b += "<li role=\"presentation\"><a clickme="+ $kav +" role=\"menuitem\" tabindex=\"-1\" href=\"#\" action=\"?action=display_sub_menu&nwp2_action="+ $av.getString("action") +"&todo=&nwp2_todo="+ $av.getString("todo") +"&container="+ $container + $axt +"&table="+ $tb + $gparams2 + (  $av.has( "params" ) ? $av.getString("params") : "" ) +"\" override-selected-record=\""+ $xid +"\" class=\"custom-single-selected-record-button\">"+ $av.getString("title") +"</a></li>";
                                        }
                            ...
                            <div class="col-md-<% out.print( $comments_column ); %>">
                                <% if( ! $static_comments.isEmpty() ){ out.print( $static_comments ); } %>
    ...
    <% }else{
        out.print("The ID and Table is not defined");
    } %>
</div>