...
<div nwp-file="<%out.print( request.getAttribute("filename") );%>" class="hyella-source-container">
    <%
        ...
        if( $error.isEmpty() ) {
            ...
            $dx = _view_details2(jdata);
            if( $dx.has("error") && ! $dx.getString("error").isEmpty() ){
                $error = $dx.getString("error");
    ...
    <div id="<% out.print( $datatable ); %>">
        <form id="workflow-filter-form" class="activate-ajax <% out.print( $cls ); %>" action="<% out.print( $action_to_perform2 ); %>" >
            ...
            <%
                if( $data.has( "info" ) && ! $data.getString( "info" ).isEmpty() ){
                    out.print( "<div class=\"row\"><div class=\"col-md-12\"><div class=\"note note-info\">" + $data.getString( "info" ) + "</div></div></div><br />" );
                }
            %>
            ...
            <%
                if( $data.has( "params2" ) && $data.getJSONObject( "params2" ).has( "status" )  ){
                    JSONObject $pstatus = $data.getJSONObject( "params2" ).getJSONObject( "status" );
                    if( $pstatus.has("option") && $pstatus.getJSONObject( "option" ).length() > 0 ){
                        $caption = $todo;
                        String $caption_label = $caption + " To";
                        if( $pstatus.has("option_meta") && $pstatus.getJSONObject("option_meta").has("label") ){
                            $caption_label = $pstatus.getJSONObject("option_meta").getString("label");
                        }
            %>
            <div class="row">
                <div class="col-md-12">
                    <label><% out.print( $caption_label ); %>  <sup>*</sup></label>
                    <select required name="nwp_wf_next_state" class="form-control">
                        <%
                            out.print( "<option value=\"\"></option>" );

                            for( Integer i1 = 0; i1 < $pstatus.getJSONObject( "option" ).names().length(); i1++ ){
                                String $ok = $pstatus.getJSONObject( "option" ).names().getString( i1 );
                                String $ov = $pstatus.getJSONObject( "option" ).getString( $ok );
