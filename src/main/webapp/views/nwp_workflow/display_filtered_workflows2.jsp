            ...
            if( dep1.has("fields")  ) {
                ...
                if( GlobalFunctions.app_user_data.has("role") ){
                    ...
                    if( GlobalFunctions.app_user_data.has("id") ){
                        ...
                        && GlobalFunctions.app_data.getJSONObject("view_my_jobs").has( jget.getString("id").trim() )
                        && GlobalFunctions.app_data.getJSONObject("view_my_jobs").getJSONObject( jget.getString("id").trim() ).has("in")
                        && ! GlobalFunctions.app_data.getJSONObject("view_my_jobs").getJSONObject( jget.getString("id").trim() ).getString("in").isEmpty()
                        ) {
                            $where += " AND ( ["+ $table.getString("table") +"].["+ dep1.getJSONObject("fields").getString("status") +"] NOT IN ( "+ GlobalFunctions.app_data.getJSONObject("view_my_jobs").getJSONObject( jget.getString("id").trim() ).getString("in") +" ) ";
                            $where += " OR ( ["+ $table.getString("table") +"].["+ dep1.getJSONObject("fields").getString("status") +"] IN ( "+ GlobalFunctions.app_data.getJSONObject("view_my_jobs").getJSONObject( jget.getString("id").trim() ).getString("in") +" )  ";
                            $where += " AND ["+ $table.getString("table") +"].[created_by] = '"+ GlobalFunctions.app_user_data.getString("id") +"' ) ) ";
                        }
                    }
                }else{
