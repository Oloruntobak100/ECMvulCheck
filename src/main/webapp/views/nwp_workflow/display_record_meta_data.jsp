...
<div nwp-file="<%out.print( request.getAttribute("filename") );%>" class="hyella-source-container">
    <%
        ...
        $table.put("table", "workflow_items" );
        if( jget.has("id") && jget.has("table") ) {
            JSONObject dep = GlobalFunctions.get_json( $table.getString("table") );
            if( dep.has("fields")  ) {
                $query.put("where", " AND ["+ dep.getJSONObject("fields").getString("reference") +"] = '"+ jget.getString("id").trim() +"' " );
                /*
                $query.put("select", "");
                $query.put("from", "");
                $query.put("join", "");
                ...
                */
                ...
                $dbs2.put("col", 4);
                //$dbs2.put("content", "Hi");
                $dbs2.put("action", "?action=display_sub_menu&nwp2_action=workflow_items&nwp2_todo=view_details_by_reference&todo=use_id&phtml_replacement_selector=" + jget.getString("html_replacement_selector"));
